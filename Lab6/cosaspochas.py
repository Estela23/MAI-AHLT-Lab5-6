...
def learnpocho(X_train, Y_train, X_val, Y_val, idx, model_name):
    ...
    Y_train = np.array([[0.0 if value!= item else 1.0 for value in range(len(np.zeros((6,)))) for item in sublist] for sublist in Y_train])
    Y_val = np.array([[0.0 if value!= item else 1.0 for value in range(len(np.zeros((6,)))) for item in sublist] for sublist in Y_val])
    print(Y_train)
    model.fit([inp1,inp2,inp3], Y_train, validation_data=([val1,val2,val3], Y_val), batch_size = 32, epochs= 1)
